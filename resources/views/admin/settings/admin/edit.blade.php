                    ...
                    <form action="{{ route('admin.settings.admin.update', $admin->id) }}" method="POST" class="pt-2 pb-2">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="email" id="email" name="email"
                                placeholder="Email Address" value="{{ old('email') ?? $admin->email }}">
                            <x-error key="email" />
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="cards[]" id="cards" class="form-select select2" multiple>
                                @foreach ($giftcards as $giftcard)
                                    <option value="{{ $giftcard->id }}" @if (in_array($giftcard->id, $selected_cards)) selected @endif>
                                        {{ $giftcard->name }}</option>
                                @endforeach
                            </select>
                            @if ($errors->has('cards'))
